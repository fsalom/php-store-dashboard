    ...
    public function getTicketsDay($day) { 
        ...
        if($daytotal<=date("Ymd")){
    		...
    		while($row = mssql_fetch_array($result)){	
                ...
                $queryDetail = "SELECT Betalingswijze, Bedrag FROM VerkoopBetaling WHERE Factuurnummer = ".$row["Factuurnummer"];
                ...
                $queryDetail = "SELECT Artikelnummer, RetourRedenID, origverkoopprijs, Verkoopprijs, Aantal, Omschrijving FROM VerkoopDetail WHERE Factuurnummer = ".$row["Factuurnummer"];
                $resultDetail = mssql_query($queryDetail) or die('Something went wrong while connecting to MSSQL: '.mssql_get_last_message());
                $total=0;
