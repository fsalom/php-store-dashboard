...
define("_BD","db396502112");
	...
	$query=mysql_query("SELECT * FROM `clients` WHERE company='".$username."' ")or die(mysql_error());
	...
	if($num>0){
        ...
        if (!$username) {
            $resp = array('ok' => false, 'msg' => '<b style="color:#990000">Debe escribir el nombre de la compañia</b>');
        } else if ($valido) {
            $resp = array("ok" => false, "msg" => '<b style="color:#990000">Nombre no disponible</b>');
        } else {
            $resp = array("ok" => true, "msg" => '<b style="color:#009900">El nombre esta disponible</b>');
        }
