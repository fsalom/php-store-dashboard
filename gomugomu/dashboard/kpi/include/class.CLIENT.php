...
class CLIENTS extends SHARE  {	 
	...
	public function searchClient($POST){
		...
		$query = "SELECT * FROM `crm_client` where (lower(name) like'%".$search."%' or lower(surname) like '%".$search."%') AND status=0 ORDER BY `surname` ASC LIMIT 10"  ;
		if($result =  $this->query($query)){
			$return = "";
			while($data=$result->fetch_assoc()){												
				...
				$return.='<a class="list_clients" href="?send=client&id_ticket='.$_POST['id_ticket'].'&id_client='.$data['id'].'">
							<div class="img"><img src="../CRM/files/'.$name.'"></div>
							'.ucwords(strtolower(strtoupper($replace_name))).' '.ucwords(strtolower(strtoupper($replace_surname))).'<br/><h3>'.$this->selectFrom($data['from']).'</h3>
						</a>';
			}
		}
		$this->closeMYSQL();
