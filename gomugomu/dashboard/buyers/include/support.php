...
function get_data_week($today,$year){
	...
	$exit=0;
			...
			//echo 'tickets:'.$id_ticket.' - '.$id_ticket_next.'<br/>';
	...
	$query_money_ES=mysql_query("SELECT SUM(quantity)
	...
	$data=mysql_fetch_array($query_money_ES);
	$value['ES_money']=$data['SUM(quantity)']+$today['ES_money'];
	
	$query_money_noES=mysql_query("SELECT SUM(quantity)
						FROM admin_dashboard.dash_payment P, admin_dashboard.dash_buyer B
						WHERE B.country <> 'ES'
						AND P.type<>'Waardebon'
						AND B.id_ticket = P.id_ticket
						AND B.id_ticket >= '".$id_ticket."'
						AND B.id_ticket <'".$id_ticket_next."'")or die(mysql_error());
	$data=mysql_fetch_array($query_money_noES);
...
function same_day(){
	...
	or die(mysql_error());
	$tickets=mysql_query("SELECT id FROM `dash_ticket` WHERE `date` LIKE '".$fecha."%' GROUP BY `id_ticket`")
	or die(mysql_error());
...
function get_answers($id_ticket){
	...
	$result=mysql_query("SELECT * FROM `dash_ticket_buyer_answer` WHERE answer != 'No Cliente' ORDER BY answer ") or die (mysql_error());
		$answer['updated']=false;
		while($row=mysql_fetch_array($result)){
			if($data['id_answer']==$row['id']){
				$answer['print'].='<option value="'.$row['id'].'" selected>'.$row['answer'].'</option>';
				$result2=mysql_query("SELECT * FROM `crm_client` WHERE id='".$data['id_client']."'") or die (mysql_error());
...
function getTotalMonth($count){
	...
	$query=mysql_query("SELECT SUM(quantity) FROM `dash_payment` WHERE
