...
define("_ALTO","200");
	...
	function get_info($option, $n, $year){
		...
		$totalOri=0;
		/*****************************************************************************
				20141110 - Modificación para que se haga la comparativa
						   del mes actual hasta el dia actual						   	
		*****************************************************************************/
		...
		$fechaActual	= 	$mesActual.'-'.$lastYear;
		echo $fechaWeb.' '.$fechaActual;
		if($fechaWeb==$fechaActual){			
			$result = $mysqli->query("SELECT subtotalOriginal, items FROM `dash_ticket` WHERE
				description<>'Waardebon'
				AND description<> 'KORTING'
...
/**************************************************/
	function target_graph2($option,$n,$year){
		
		$value	=	get_target($option,$n,$year);
		$week	=	get_week($n,$year);			
		$month	=	get_month2($n,$year);
		...
		$content['total']=number_format($totalm,2,',','.').'€';
...
/**************************************************/
	function get_option($option, $n,$y){
		...
		$NOW_year=date('Y', strtotime($fyear."W".$week."2"));
		//echo $fyear.'-W'.$week.' NOW:'.$NOW_year;
		if($NOW_year>$fyear)
			$fyear=$NOW_year;
		$select=
		'<form method="GET">
...
/**************************************************/
	function get_week($week_number,$year){
		...
		if($week_number<10 && $week_number[0]!="0")
...
/**************************************************/
	function get_month($m, $year){
		...
		for($day=1;$day<=$n;$day++){
			...
			$month[$day]['date']=$date;
			$total+=get_quantity($date);
			//echo '>>>>> DAY :'.$date.' - '.get_quantity($date).'<br/>';
...
/**************************************************/
	function get_month2($m, $year){
		...
		$comparar=0;
		$fecha_IN	=	($year).'-'.$m;
		$year_OUT	= 	date ('Y',time());
		$fecha_OUT	=	($year_OUT-1).date('-m',time());
		//echo $fecha_IN.' IN - OUT '.$fecha_OUT.'<br/>';
