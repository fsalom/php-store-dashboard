...
class BECOSOFT {
    ...
    public function getReference($reference){     	
    	...
    	$support_season =  $data['Seizoen'];    	
    	$data2			= new ITEM($data[2],$data['MatrixV'],$data['MatrixH'], $data['Referentienummer'], $data['Seizoen'], $this->getStock($data[2],$data['MatrixH'])); 	    	   	
    	return $data2;
    }
    ...
    public function getTicketDetails($id){    	
    	...
    	$query = "SELECT Artikelnummer,RetourRedenID, Omschrijving, Verkoopprijs,origverkoopprijs,Aantal FROM VerkoopDetail WHERE Factuurnummer = ".$id;
    	$result = mssql_query($query) or die(mssql_get_last_message());
    	$x=0;	    	
    ...
    public function getTicketsDay($day) {    	
	 ...
	 public function getStock($desciption,$size){    	
	    
	    $data = "";
	    //echo $desciption." ".$size."<br/>";
	    $query = "SELECT l.stock
