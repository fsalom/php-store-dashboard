...
class GOMUGOMU extends FUNCIONES  {
    ...
    public function getTicketDetails($id){    	
    	...
    	while($row = mysql_fetch_array($result)){ 	
    		$item[$i] = new ARTICLE(
						    		$this->getReference($row['articlenr']), //name, colour, size, reference, season   		    		    	
						    		$this->getNameReturn(0),   		
						    		str_replace(array("Superdry", "SUPERDRY"),'',$row["description"]),
									$row["subtotal"]*$row["items"],
									$row['subtotalOriginal']*$row["items"],
									$row["items"],
									$this->getMargenBruto($row['subtotalOriginal'],$row["subtotal"],$this->support_season),  //money, percentage 			   
    ...
    public function getTicketByReference($reference){    	
    	$this->connectMYSQL();
    	$query = "SELECT * FROM dash_ticket WHERE referencenr = '".$reference."' ORDER BY `colour`, `size`";
